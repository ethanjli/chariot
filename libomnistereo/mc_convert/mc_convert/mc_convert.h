 ...
 */
template<typename iter_t> mxArray* values_to_new_mxArr (iter_t beg, iter_t end)
{
  using namespace std;
  
  //iterator_traits<iter_t> a; // geht
  //iterator_traits<iter_t>::pointer b; // geht nicht
  typedef typename iterator_traits<iter_t>::value_type T;
  typedef typename iterator_traits <iter_t>::pointer ptr;
  //typedef iterator_traits<iter_t> IT;
