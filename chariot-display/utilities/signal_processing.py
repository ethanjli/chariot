...
class SlidingWindowThresholdFilter(SlidingWindowFilter):
    def __init__(self, window_size=10, threshold=1, nonstationary_transition_smoothness=2,
                 stationary_transition_smoothness=2,
                 #smoothing_mode=('convolve', gaussian_window(7, 2.0)), estimation_mode=('poly', 5)):
                 smoothing_mode=None, estimation_mode=('kernel', half_gaussian_window(10, 2.0))):
        ...
        self.stationary_transition_smoothness = stationary_transition_smoothness
    ...
    def estimate_current(self):
        if self.in_stationary_range(self.get_head()):
            ...
            if self._stationary_duration >= self.stationary_transition_smoothness:
                self._nonstationary_duration = 0
                return self._estimate_in_stationary()
            else:
                return self._estimate_in_nonstationary()
        else:
            self._nonstationary_duration += 1
            if self._nonstationary_duration >= self.nonstationary_transition_smoothness:  # In nonstationary state
                self._stationary_value = None
                self._stationary_duration = 0
                return self._estimate_in_nonstationary()
            else:
    ...
    def _estimate_in_stationary(self):
        if self._stationary_value is None:  # Initialize stationary value
            self._stationary_value = super(SlidingWindowThresholdFilter, self).estimate_current()
        return self._stationary_value

    def _estimate_in_nonstationary(self):
...
class ThresholdKalmanFilter(KalmanFilter):
    def __init__(self, position_from_stationary=5, velocity_from_stationary=5,
                 acceleration_from_stationary=8, velocity_to_stationary=5,
                 acceleration_to_stationary=5, reset_velocity=150, reset_acceleration=150):
    ...
    def append(self, x):
        ...
        if self._stationary_value is None:
            ...
            if (abs_position > self.position_from_stationary
                ...
                self._stationary_value = None
        if abs_velocity > self.reset_velocity or abs_acceleration > self.reset_acceleration:
            self.init_kalman()
            self._stationary_value = None
            self.estimated = self.kalman.predict().ravel()
