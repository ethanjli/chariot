#!/bin/bash
exec 1>logs/gen_logs.txt 2>logs/gen_err.txt
HOME_DIR=`pwd`
...
BROX_MALIK="$CVFISH/data/Log_C920_x1/3_downsampled"
CODE=$CVFISH
...
mkdir $RECORD/$strt
R_INPUT="$RECORD/$strt/Input"
R_MOTSEG="$RECORD/$strt/Motseg"
R_RECON="$RECORD/$strt/DepthReconstruction"

INPUT="$BROX_MALIK/subsequence_start${strt}_length${length}_subsampling${subsample}"
RESULTS="$BROX_MALIK/broxmalik_Size${size}_start${strt}_length${length}_subsampling${subsample}/broxmalikResults"
...
BM_RESULTS="$RESULTS/broxmalikTracks${tracks}.dat"
MOTSEG_RESULTS="$RESULTS/f1t${tracks}/v1/vw10_nn10_k1_thresh100_max_occ2_op0_cw2.5/init200/mdl20000_pw3000_oc10_engine0_it5"
...
cd $CODE
python -m video_popup.depth_reconstruction.depth_reconstruction_test --seg_file $MOTSEG_RESULTS/results.pkl --endframe ${tracks} 1>$LOG_DIR/recon_logs.txt 2>$LOG_DIR/recon_err.txt
cd $HOME_DIR
